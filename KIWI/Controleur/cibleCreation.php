...
if (!empty($_POST['id_piece']) AND !empty($_POST['choix_piece']) AND !empty($_POST['type']) AND !empty($_POST['superficie']) AND !empty($_POST['nb_capteur']) AND !empty($_POST['id_maison'])){
    ...
    $db->exec('INSERT INTO piece VALUES("'.$_POST['id_piece'].'", "'.$_POST['choix_piece'].'", "'.$_POST['type_pièce'].'", "'.$_POST['superficie'].'", "'.$_POST['nb_capteur'].'", "'.$_POST['id_maison'].'")') or die(print_r($db->errorInfo()));
